...
static int wdc_de_unlock_drive(int fd, bool unlock, char *key)
{
	...
	if(unlock) {
		dataBufferSize = strlen(key);
		memcpy(&dataBuffer, key, dataBufferSize);
	} else {
		dataBufferSize = strlen(key);
		memcpy(&dataBuffer, key, dataBufferSize);
	}

	ret = nvme_set_feature(fd, WDC_DE_GLOBAL_NSID, WDC_DE_VU_UNLOCK_FEATURE_IDENTIFIER, dataBuffer, 0,
			0, 0, &result);
...
static int wdc_do_drive_essentials(int fd, char *dir, char *key)
{
	...
	/* Unlock drive */
	ret = wdc_de_unlock_drive(fd, true, key);
	if (ret) {
		fprintf(stderr, "ERROR : WDC : unlock drive failed, ret = %d\n", ret);
		return -1;
	}
	...
	/* Relock drive */
	ret = wdc_de_unlock_drive(fd, false, WDC_DE_VU_LOCK_PASSPHRASE);
	if (ret) {
		fprintf(stderr, "ERROR : WDC : relock drive failed, ret = %d\n", ret);
		return -1;
	}
		...
		struct plugin *plugin)
{
	...
	const struct argconfig_commandline_options command_line_options[] = {
		...
		{"key", 'k', "KEY", CFG_STRING, &cfg.key, required_argument, key},
		{ NULL, '\0', NULL, CFG_NONE, NULL, no_argument, desc},
		{NULL}
	};
	...
	if (cfg.key != NULL) {
		strncpy(k, cfg.key, PATH_MAX);
	} else {
		fprintf(stderr, "ERROR : WDC : No drive key specified\n");
		return -1;
	}
