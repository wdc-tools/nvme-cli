...
static int wdc_do_get_sn730_log_len(int fd, uint32_t *len_buf, uint32_t subopcode)
{
	...
	admin_cmd.addr = (__u32)(uintptr_t)output;
	admin_cmd.cdw12 = subopcode;
	admin_cmd.cdw10 = SN730_LOG_CHUNK_SIZE / 4;

	ret = nvme_submit_passthru(fd, NVME_IOCTL_ADMIN_CMD, &admin_cmd);
	if (ret == 0)
		memcpy(len_buf, output, sizeof(uint32_t));
	free(output);
	return ret;
}
...
static int wdc_do_sn730_get_and_tar(int fd, char * outputName)
{
	...
	tar_metadata* myBuffer = NULL;

	myBuffer = (struct tar_metadata*) malloc(sizeof(tar_metadata));
	if (myBuffer == NULL) {
		fprintf(stderr, "ERROR : WDC : malloc : %s\n", strerror(errno));
		ret = -1;
		goto free_buf;
	}
	memset(myBuffer, 0, sizeof(tar_metadata));

	/* Create Logs directory  */
	wdc_UtilsGetTime(&myBuffer->timeInfo);
	memset(myBuffer->timeString, 0, sizeof(myBuffer->timeString));
	wdc_UtilsSnprintf((char*)myBuffer->timeString, MAX_PATH_LEN, "%02u%02u%02u_%02u%02u%02u",
			myBuffer->timeInfo.year, myBuffer->timeInfo.month, myBuffer->timeInfo.dayOfMonth,
			myBuffer->timeInfo.hour, myBuffer->timeInfo.minute, myBuffer->timeInfo.second);

	wdc_UtilsSnprintf((char*)myBuffer->bufferFolderName, MAX_PATH_LEN, "%s",
			(char*)outputName);

	retPtr = getcwd((char*)myBuffer->currDir, MAX_PATH_LEN);
	if (retPtr != NULL)
		wdc_UtilsSnprintf((char*)myBuffer->bufferFolderPath, MAX_PATH_LEN, "%s%s%s",
				(char *)myBuffer->currDir, WDC_DE_PATH_SEPARATOR, (char *)myBuffer->bufferFolderName);
	else {
		fprintf(stderr, "ERROR : WDC : get current working directory failed\n");
		return -1;
	}

	ret = wdc_UtilsCreateDir((char*)myBuffer->bufferFolderPath);
	if (ret != 0)
	{
		fprintf(stderr, "ERROR : WDC : create directory failed, ret = %d, dir = %s\n", ret, myBuffer->bufferFolderPath);
		return -1;
	} else {
		fprintf(stderr, "Stored log files in directory: %s\n", myBuffer->bufferFolderPath);
	}

	if (wdc_do_get_sn730_log_len(fd, &full_log_len, SN730_GET_FULL_LOG_LENGTH) ||
		wdc_do_get_sn730_log_len(fd, &key_log_len, SN730_GET_KEY_LOG_LENGTH) ||
		wdc_do_get_sn730_log_len(fd, &core_dump_log_len, SN730_GET_COREDUMP_LOG_LENGTH) ||
		wdc_do_get_sn730_log_len(fd, &extended_log_len, SN730_GET_EXTENDED_LOG_LENGTH)) {

		fprintf(stderr, "NVMe Status:%s(%x)\n", nvme_status_to_string(ret), ret);
		return ret;
	}

	full_log_buf= (__u8*) malloc(sizeof (__u8) * full_log_len);
	key_log_buf= (__u8*) malloc(sizeof (__u8) * key_log_len);
	core_dump_log_buf= (__u8*) malloc(sizeof (__u8) * core_dump_log_len);
	extended_log_buf= (__u8*) malloc(sizeof (__u8) * extended_log_len);
	...
	memset(full_log_buf, 0, full_log_len);
	memset(key_log_buf, 0, key_log_len);
	memset(core_dump_log_buf, 0, core_dump_log_len);
	memset(extended_log_buf, 0, extended_log_len);
	...
	/* Write log files */
	wdc_UtilsSnprintf(myBuffer->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin", (char*)myBuffer->bufferFolderPath, WDC_DE_PATH_SEPARATOR,
			"full_log", (char*)myBuffer->timeString);
	wdc_WriteToFile(myBuffer->fileName, (char*)full_log_buf, full_log_len);

	wdc_UtilsSnprintf(myBuffer->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin", (char*)myBuffer->bufferFolderPath, WDC_DE_PATH_SEPARATOR,
			"key_log", (char*)myBuffer->timeString);
	wdc_WriteToFile(myBuffer->fileName, (char*)key_log_buf, key_log_len);

	wdc_UtilsSnprintf(myBuffer->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin", (char*)myBuffer->bufferFolderPath, WDC_DE_PATH_SEPARATOR,
			"core_dump_log", (char*)myBuffer->timeString);
	wdc_WriteToFile(myBuffer->fileName, (char*)core_dump_log_buf, core_dump_log_len);

	wdc_UtilsSnprintf(myBuffer->fileName, MAX_PATH_LEN, "%s%s%s_%s.bin", (char*)myBuffer->bufferFolderPath, WDC_DE_PATH_SEPARATOR,
			"extended_log", (char*)myBuffer->timeString);
	wdc_WriteToFile(myBuffer->fileName, (char*)extended_log_buf, extended_log_len);

	/* Tar the log directory */
	wdc_UtilsSnprintf(myBuffer->tarFileName, sizeof(myBuffer->tarFileName), "%s%s", (char*)myBuffer->bufferFolderPath, WDC_DE_TAR_FILE_EXTN);
	wdc_UtilsSnprintf(myBuffer->tarFiles, sizeof(myBuffer->tarFiles), "%s%s%s", (char*)myBuffer->bufferFolderName, WDC_DE_PATH_SEPARATOR, WDC_DE_TAR_FILES);
	wdc_UtilsSnprintf(myBuffer->tarCmd, sizeof(myBuffer->tarCmd), "%s %s %s", WDC_DE_TAR_CMD, (char*)myBuffer->tarFileName, (char*)myBuffer->tarFiles);

	ret = system(myBuffer->tarCmd);
		...
		struct plugin *plugin)
{
	...
	if ((capabilities & WDC_DRIVE_CAP_D0_LOG_PAGE) == WDC_DRIVE_CAP_D0_LOG_PAGE) {
		...
		if (ret) {
			fprintf(stderr, "ERROR : WDC : Unable to read D0 Log Page\n");
			return ret;
		}
	}
	else {
